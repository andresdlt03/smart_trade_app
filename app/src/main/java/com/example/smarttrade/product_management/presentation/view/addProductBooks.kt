...
fun addProductBooks(viewModel: AddProductBookViewModel, navHostController: NavHostController, vm: catalogueViewModel){
    ...
    TopBar({viewModel.goBackToCategories(navHostController)},navHostController)
    ...
    OutlinedText(
        value = bookState.value.isbn,
        label = "ISBN",
        upDateField = {viewModel.updateIsbn(it)},
        errorMessage = bookState.value.isbnError,
    )
    Spacer(modifier = Modifier.height(14.dp))

...
@HiltViewModel
class AddProductBookViewModel : ViewModel(){
    ...
    fun checkAllVariables() {
        ...
        if (_state.value.name.isNullOrEmpty()) {
            ...
            _state.value.name = "El nombre está vacío"
            return
            ...
            _state.value.checkVariables = false
            _state.value.textError = "El ISBN está vacío"
            return
        } else if (_state.value.isbn.length != 13) {
            _state.value.checkVariables = false
            _state.value.textError = "El ISBN debe tener 13 caracteres"
            return

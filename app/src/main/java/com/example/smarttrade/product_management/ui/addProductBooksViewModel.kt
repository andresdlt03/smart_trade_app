...
class `addProductBooksViewModel` : ViewModel(){
    ...
    fun checkAllVariables() {
        ...
        if (_name.value.isNullOrEmpty()) {
            ...
            return
        } else if (_isbn.value.isNullOrEmpty()) {
            _checkVariables.value = false
            _textError.value = "El ISBN está vacío"
            return
        } else if (_isbn.value!!.length != 13) {
            _checkVariables.value = false
            _textError.value = "El ISBN debe tener 13 caracteres"
            return
        } else if (_price.value.isNullOrEmpty()) {
            _checkVariables.value = false
            _textError.value = "El precio está vacío"
            return
        } else {
            val precioFloat = _price.value!!.toFloatOrNull()
            if (precioFloat == null) {
                _checkVariables.value = false

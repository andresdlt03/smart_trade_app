...
class `addProductTechnologyViewModel` : ViewModel(){
    ...
    fun checkAllVariables(){
        ...
        if (_name.value.isNullOrEmpty()) {
            ...
            return
        } else if (_model.value.isNullOrEmpty()) {
            _checkVariables.value = false
            _textError.value = "El modelo está vacío"
            return
        } else if (_model.value!!.length > 20) {
            _checkVariables.value = false
            _textError.value = "El modelo es demasiado largo"
            return
        } else if (_energy.value.isNullOrEmpty()) {
            _checkVariables.value = false
            _textError.value = "El consumo energético está vacío"
            return
        } else {
            val energyFloat = _energy.value!!.toFloatOrNull()
            if (energyFloat == null) {
                _checkVariables.value = false
                _textError.value = "El consumo energético debe ser un número"
                return
            }
        }

        if (_price.value.isNullOrEmpty()) {
            _checkVariables.value = false
            _textError.value = "El precio está vacío"
            return
        } else {
            val priceFloat = _price.value!!.toFloatOrNull()
            if (priceFloat == null) {
                _checkVariables.value = false
                _textError.value = "El precio debe ser un número"
                return
            }
        }
